"""Task query tools for Minimax API."""
...
async def minimax_get_task(
    task_id: Annotated[
        str,
        Field(
            description="The task ID returned from a generation request. This is the 'task_id' field from any minimax_generate_* tool response."
        ),
    ],
) -> str:
...
@mcp.tool()
async def minimax_get_tasks_batch(
    task_ids: Annotated[
        list[str],
