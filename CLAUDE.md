...
MCP (Model Context Protocol) server for MiniMax H3 multimodal video generation via AceDataCloud API.
...
core/
  ...
  types.py      — Literal types (MinimaxModel, etc.)
  exceptions.py — Error classes (AuthError, APIError, TimeoutError)
  utils.py      — Formatting helpers
tools/
  video_tools.py   — generate video from text, images, or audio guidance
...
1. **Source of truth** — `_docs/openapi/minimax.json` is the OpenAPI spec for the MinimaxMCP API.
