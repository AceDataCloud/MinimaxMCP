"""
Integration tests for Minimax MCP Server.
...
class TestVideoTools:
    ...
    async def test_generate_video_basic(self):
        ...
        from tools.video_tools import minimax_generate_video_from_text

        result = await minimax_generate_video_from_text(
...
class TestInfoTools:
    ...
    async def test_list_models(self):
        """Test minimax_list_models tool."""
        from tools.info_tools import minimax_list_models

        result = await minimax_list_models()
    ...
    async def test_list_actions(self):
        """Test minimax_list_actions tool."""
        from tools.info_tools import minimax_list_actions

        result = await minimax_list_actions()

        print("\n=== List Actions Result ===")
        print(result)

        assert "minimax_generate_video_from_text" in result
        assert "minimax_get_task" in result
...
class TestTaskTools:
    ...
    async def test_get_task_with_real_id(self):
        ...
        from tools.task_tools import minimax_get_task
        from tools.video_tools import minimax_generate_video_from_text

        gen_result = await minimax_generate_video_from_text(
        ...
        if "Task ID:" in gen_result:
            ...
            if task_id and task_id != "N/A":
                ...
                task_result = await minimax_get_task(task_id)
                print(task_result)
